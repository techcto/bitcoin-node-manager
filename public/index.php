...
define('CHAIN_SYMBOL', $_ENV["CHAIN_SYMBOL"] ?? 'BTC');
define('RPC_USER', $_ENV["RPC_USER"] ?? 'litecoin');
define('RPC_PASSWORD', $_ENV["RPC_PASSWORD"] ?? 'password');
define('RPC_PORT', $_ENV["RPC_PORT"] ?? '9332');
define('RPC_IP', $_ENV["RPC_IP"] ?? '127.0.0.1');
	...
	if(isset($_GET['c']) && $_GET['t'] == $_SESSION["csfrToken"]){
		...
		// Add Hoster Command
			...
			if(preg_match("/^[0-9a-zA-Z-,\. ]{3,40}$/", $_GET['n'])) {
				$hosterJson = file_get_contents($GLOBALS["data-dir"].'/hoster.json');
				$hoster = json_decode($hosterJson);
				$hoster[] = $_GET['n'];
				file_put_contents($GLOBALS["data-dir"].'/hoster.json',json_encode($hoster));
...
// Hoster Page	
		...
		if($_GET['c'] == "remove"){
			if(preg_match("/^[0-9a-zA-Z-,\. ]{3,40}$/", $_GET['n'])) {
				if(($key = array_search($_GET['n'], $hosterList)) != false) {
					...
					file_put_contents($GLOBALS["data-dir"].'/hoster.json',json_encode($hosterList)); 
	...
	$content = json_decode(file_get_contents($GLOBALS["data-dir"].'/hoster.json'), TRUE);
